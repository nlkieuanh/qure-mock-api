export default async function handler(req, res) {
  ...
  const fail = (status, payload) => res.status(status).json({ usecases: [], ...payload });
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      console.error("[/api/usecases] upstream error:", resp.status, detail);
    ...
    for (const ad of items) {
      ...
      const arr =
        Array.isArray(uc) ? uc :
        (typeof uc === "string" && uc.trim() ? [uc.trim()] : ["Unknown"]);
    ...
    return res.status(200).json({ usecases: Array.from(map.values()) });
  } catch (err) {
